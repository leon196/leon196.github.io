...
{    
    constructor(canvas)
    {
        ...
        this.uniforms = {
            ...
            alphabet: twgl.createTexture(gl, {
                src: "common/font-octavio-good.png",
                flipY: true,
                minMag: gl.LINEAR
            }),
            bluenoise: twgl.createTexture(gl, {
                src: "common/bluenoise-shadertoy.png",
                flipY: true
            }),
            final_pass: false,
        };
    ...
    // {
        ...
        var self = this;
        var name = "bim-boom-bam.frag";
        loadFiles("shader", [name], "text", function(data)
        {
            self.filter = data[name];

            // shader set up
            self.material = twgl.createProgramInfo(self.gl, [self.vertex, self.filter]);

            // main loop
            requestAnimationFrame((time) => self.loop(time));
        });
    }
