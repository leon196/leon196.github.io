...
void main() 
{
    ...
    float t = sin(uTimeElapsed * 0.001) * 0.5 + 0.5;
   	...
   	// vec2 force = neighborhood(pixelize(uv, 4.0 + 4.0 * rand(color.rg + color.b)), uVideo);
   	vec2 force = neighborhood(vTexCoord, uFramebuffer);
   	// vec2 force = neighborhood(pixelize(uv, 32.0), uVideo);
